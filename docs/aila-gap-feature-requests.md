...
`go-agent` is a strong fit for Aila's model/tool runtime core because it provides a stream-first runner, tool dispatch, sessions, policy hooks, retries, event sinks, canonical events, per-run overrides and aggregate limits, replayable JSON events with optional correlation, observable policy pending and recoverable denials, streaming tool progress, richer tool results, optional model capability hints, and an OpenAI-compatible Chat Completions adapter.

Remaining product work for Aila stays in the terminal layer: FSM, TUI, config, artifact resolver, permission UX, additional provider adapters, and orchestration patterns outside the library core described in this document.

## Gap summary

Baseline audit before the go-agent “Aila Runtime Gap Features” plan; the **Status after go-agent** matrix below is the authoritative post-plan resolution for library core.

| ID          | Feature request                                                    | Priority     | Why Aila needs it                                                                                                                                     | Current go-agent status (post-plan core)                                                                                 |
| ----------- | ------------------------------------------------------------------ | ------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| GA-AILA-001 | Run-scoped instructions and tool exposure                          | **Critical** | Aila's FSM requires state-specific prompts and state-limited tool visibility per phase.                                                               | `RunRequest.Instructions`, `ToolNames`, and `Limits` / `RunLimits`; runner resolves against tools registered on `Agent`. |
| GA-AILA-002 | Recoverable policy denials for tool calls                          | **Critical** | Aila must return approval denial/defer results to the active capability instead of always terminating the run.                                        | Optional `PolicyDecision.ToolResult` supplies a synthetic tool result on denial so the run can continue when configured. |
| GA-AILA-003 | Observable policy request lifecycle                                | **High**     | Aila's TUI needs to show an approval prompt while a decision is pending, not only after a policy decision exists.                                     | `EventPolicyPending` is emitted before synchronous `Policy.Decide` for explicit policies.                                |
| GA-AILA-004 | Streaming tool progress events                                     | **High**     | Aila's `bash`, `fetch`, and edit-like tools need visible progress, partial output, cancellation, and no hidden long-running jobs.                     | `StreamingTool` / `ToolProgressEmitter` / `EventToolProgress` with bounded incremental emits.                            |
| GA-AILA-005 | Richer tool result authoring and metadata                          | **High**     | Aila tool results need compressed summaries, exact source refs, truncation facts, and structured JSON. Helper tools cannot return structured results. | Extended `ToolResult` (metadata, truncation/compression/source, `Opaque`) with `ValidateToolResult` before emission.     |
| GA-AILA-006 | Additional provider adapter packages                               | **High**     | Aila plans OpenAI Realtime, custom/OpenAI-compatible APIs, Anthropic, and several plan providers.                                                     | Still only OpenAI-compatible Chat Completions SSE in-tree; further adapters remain host or separate packages.            |
| GA-AILA-007 | Optional model capability metadata                                 | **Medium**   | Aila's `models` command and header need selected-model context window, tool support, reasoning support, and token accounting facts.                   | `ModelCapabilitiesOf` / `ModelCapabilitiesProvider`; `openai.ChatModel` exposes static hints.                            |
| GA-AILA-008 | Replay-safe event serialization                                    | **Medium**   | Aila saves session history, runs, approvals, command output, reviews, undo data, and compacted context under `.aila/`.                                | `MarshalEvents` / `UnmarshalEvents` JSON envelope v1 with string errors and replay tests.                                |
| GA-AILA-009 | Parent/child run event correlation for utility work and sub-agents | **Medium**   | Aila needs foreground model runs, utility model jobs, and orchestrated sub-agent runs to be multiplexed without losing lineage.                       | Optional `RunID`, `ParentRunID`, `TaskID`, `Metadata` on `RunRequest` and stamped on `Event`.                            |
| GA-AILA-010 | Run-level limits beyond model steps                                | **Medium**   | Aila needs bounded autonomy for tool calls, wall-clock duration, output volume, and budget-sensitive execution.                                       | `RunLimits` caps tool-call count, wall-clock duration, and cumulative tool output bytes alongside model steps.           |

### Status after go-agent “Aila Runtime Gap Features” plan (library core)

The sequenced plan in `.agentera/plan.yaml` (tasks 1–6) landed the following **in core go-agent** so Aila can plan product work on top without re-opening these runtime seams. **Deferred** items stay host-owned extensions or explicitly non-goals per project direction.

| ID          | Resolution in go-agent core                                                                                                            |
| ----------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| GA-AILA-001 | **Addressed** — `RunRequest.Instructions`, `ToolNames`, and `Limits` / `RunLimits` with runner resolution and tests.                   |
| GA-AILA-002 | **Addressed** — `PolicyDecision.ToolResult` supplies synthetic tool results on denial so the run can continue.                         |
| GA-AILA-003 | **Addressed** — `EventPolicyPending` before synchronous `Policy.Decide` for explicit policies.                                         |
| GA-AILA-004 | **Addressed** — `StreamingTool.CallStream`, `ToolProgressEmitter`, `EventToolProgress`, bounded emits.                                 |
| GA-AILA-005 | **Addressed** — Extended `ToolResult` plus `ValidateToolResult` before emitting events.                                                |
| GA-AILA-006 | **Deferred** — Only the OpenAI-compatible Chat Completions SSE adapter ships in-tree; more provider packages remain separate adapters. |
| GA-AILA-007 | **Addressed** — `ModelCapabilitiesOf` / `ModelCapabilitiesProvider`; `openai.ChatModel` implements (static hints only).                |
| GA-AILA-008 | **Addressed** — `MarshalEvents` / `UnmarshalEvents` JSON envelope v1.                                                                  |
| GA-AILA-009 | **Addressed** — Optional `RunID`, `ParentRunID`, `TaskID`, `Metadata` on `RunRequest` and stamped on `Event`.                          |
| GA-AILA-010 | **Addressed** — `RunLimits` on `RunRequest` for tool-call count, duration, and cumulative tool output caps.                            |
