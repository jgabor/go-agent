...
- `TurnResult`, `EventUsage`, and `RunResult.Usage` carry typed token/cache/request/provider/model usage facts. Cost and pricing remain Lira-owned product policy.
- `ModelFromSimple` keeps tests and local models concise without making the core turn-first again.

The replacement conclusion is unchanged: Lira should keep provider registry, credential discovery, auth/cache behavior, CLI subprocess providers, workflow policy, workdir behavior, pricing, and user-visible CLI semantics. The remaining generic `go-agent` gaps are now narrower: broader optional provider packages beyond OpenAI-compatible Chat Completions, provider HTTP option surfaces that remain generic enough for a library, and any Lira-side adapter/parity tests.
...
| OpenAI-compatible chat adapter | Started      | `providers/openai.ChatModel` implements `goagent.Model`; it streams Chat Completions SSE into canonical events.                                        |
...
| `MaxTokens uint`             | Provider max output control                         | `TurnOptions.MaxOutputTokens` exists; adapter mapping is provider-specific.                      |
| `Temperature float64`        | Sampling control                                    | `TurnOptions.Temperature` exists.                                                                |
| `ReasoningEffort string`     | Provider reasoning control                          | `providers/openai.ChatOptions.ReasoningEffort` exists; broader provider policy remains external. |
...
| ------------------------------------------ | --------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| Provider-level streaming contract          | Closed in go-agent runtime  | Lira's worker calls `StreamComplete` as the main path, and API drivers parse provider SSE streams. `go-agent.Model` is now stream-first and runtime results assemble from canonical events. OpenAI-compatible Chat Completions SSE is implemented in `providers/openai`; other provider families remain adapter/package work.      | Lira: `/home/jgabor/git/lira/internal/driver/types.go:12-15`, OpenAI SSE `/home/jgabor/git/lira/internal/driver/openai.go:210-280`, chat SSE `/home/jgabor/git/lira/internal/driver/chatcompletions.go:205-303`, Anthropic SSE `/home/jgabor/git/lira/internal/driver/anthropic.go:211-340`. go-agent: `Model.Stream` in `/home/jgabor/git/go-agent/api.go`; stream assembly in `/home/jgabor/git/go-agent/stream_assembly.go`; OpenAI-compatible Chat Completions SSE in `/home/jgabor/git/go-agent/providers/openai/openai.go`. | `go-agent` owns the generic stream seam and the optional Chat Completions adapter; provider packages or Lira own other concrete wire parsers. |
| Turn options for model parameters          | Partially closed            | Lira depends on max tokens, temperature, and reasoning effort across API and CLI providers. `go-agent.TurnRequest` now carries provider-neutral max output tokens, temperature, and stop sequences; OpenAI-compatible reasoning effort remains in the typed `providers/openai.ChatOptions` surface.                                | Lira request fields in `/home/jgabor/git/lira/internal/driver/types.go:17-30`; go-agent `TurnOptions` in `/home/jgabor/git/go-agent/api.go`; OpenAI `ChatOptions` in `/home/jgabor/git/go-agent/providers/openai/openai.go`.                                                                                                                                                                                                                                                                                                      | `go-agent` for narrow generic model options; provider packages or Lira for provider-specific mapping.                                         |
| Usage and cost metadata                    | Partially closed            | Lira accumulates token usage and cost across worker steps and exposes provider/accounting behavior. `go-agent` now carries typed usage facts, but not cost/pricing policy.                                                                                                                                                         | Lira response in `/home/jgabor/git/lira/internal/driver/types.go:32-36`; worker accumulation in `/home/jgabor/git/lira/internal/worker/worker.go`; go-agent `Usage`, `EventUsage`, and `RunResult.Usage` in `/home/jgabor/git/go-agent/api.go`.                                                                                                                                                                                                                                                                                   | `go-agent` for typed usage facts; Lira for pricing rules and product accounting.                                                              |
...
| Provider adapter streaming implementation  | Closed for Chat Completions | The `go-agent` OpenAI-compatible adapter now sends Chat Completions requests with `stream: true`, parses SSE records, accumulates indexed tool-call deltas, emits provider-supplied usage, reports bounded diagnostics, and rejects non-SSE success responses instead of adapting complete JSON as a stream.                       | go-agent OpenAI adapter in `/home/jgabor/git/go-agent/providers/openai/openai.go`; provider tests in `/home/jgabor/git/go-agent/providers/openai/openai_test.go`; Lira SSE implementations listed above.                                                                                                                                                                                                                                                                                                                          | `go-agent` for the optional Chat Completions adapter; other provider families remain future adapter work.                                     |
| Provider HTTP option surface               | Medium                      | Lira provider code handles base URL path differences, custom headers, keyless auth, bearer/x-api-key differences, special model routing, and timeout differences. `go-agent.providers/openai.ChatModel` exposes model, API key, base URL, HTTP client, and typed Chat Completions options, but not Lira's provider-policy surface. | go-agent `ChatModel` fields in `/home/jgabor/git/go-agent/providers/openai/openai.go`; Lira provider files under `/home/jgabor/git/lira/internal/driver`.                                                                                                                                                                                                                                                                                                                                                                         | `go-agent` for generic adapter options; Lira for provider-specific policy.                                                                    |
...
| Behavior                                        | Lira evidence                                                                          | go-agent status                                                                                                |
| ----------------------------------------------- | -------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- |
| Streaming `stream: true` with `[DONE]` parsing  | `/home/jgabor/git/lira/internal/driver/chatcompletions.go:147-153`, `205-303`          | Implemented for the already-selected OpenAI-compatible `providers/openai.ChatModel`.                           |
| Incremental tool argument accumulation by index | `/home/jgabor/git/lira/internal/driver/chatcompletions.go:223-294`                     | Implemented for Chat Completions SSE using canonical tool-call deltas.                                         |
| Final usage capture                             | `/home/jgabor/git/lira/internal/driver/chatcompletions.go:326-343`                     | Implemented when provider chunks supply usage; no cost is synthesized.                                         |
| `reasoning_effort` request field                | `/home/jgabor/git/lira/internal/driver/chatcompletions.go:38-47`                       | Implemented as typed `providers/openai.ChatOptions.ReasoningEffort`.                                           |
...
- Streaming, usage, indexed tool-call deltas, bounded diagnostics, and typed reasoning effort are closed for the optional OpenAI-compatible Chat Completions adapter.
...
| Anthropic content block mapping         | `/home/jgabor/git/lira/internal/driver/anthropic.go:393-464`                                                                | Generic message blocks exist; Anthropic-specific mapping is missing. |
...
- OpenAI Chat Completions streaming (already implemented for `providers/openai`),
- OpenAI Responses,
- Anthropic Messages,
...
- Still requires more provider packages and generic-enough HTTP option surfaces before provider packages can be faithful across Lira's full API surface.
...
1. Add only additional generic turn options when they are proven provider-neutral; max output tokens, temperature, and stop sequences already exist, while provider-specific knobs such as reasoning effort belong in typed provider-package options.
2. Extend optional provider packages only where the core data model can represent their behavior faithfully.
3. Add concrete provider SSE parser tests where additional provider packages support token/tool-call streaming; OpenAI-compatible Chat Completions already has focused SSE fidelity and failure tests.
