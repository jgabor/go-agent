...
- OpenAI-compatible hidden reasoning is replay-only provider state attached to the assistant message that produced it. `Message.HiddenReasoning` is omitted from default JSON/event serialization; `reasoning_content` is replayed only with the corresponding assistant tool-call message, including retained multi-subturn and later-user-turn histories.
- OpenAI-compatible thinking controls are typed and finite in `providers/openai.ChatOptions` (`ThinkingEnabled`, `ThinkingDisabled`, low/medium/high `ReasoningEffort`); invalid values fail before provider requests. Usage exposes `Usage.ReasoningTokens` as a typed count only, with no pricing or cost semantics.
- Per-run controls live on `RunRequest` (`Instructions`, `Tools`, `ToolNames`, `Limits`) with distinct stop reasons for tool-call count, cumulative tool output, and wall-clock duration when configured. When `Tools` is non-nil it replaces the agent tool base for that run; names must not overlap agent-registered tools. `ToolNames` subsets the effective tool map; duplicate or missing names error at run start. Context cancellation or wall-clock run limits during a blocking `Policy.Decide` classify as cancel or duration limit, not `StopPolicyDenied`.
- `providers/openai.ChatModel` capability hints merge integration identity with a curated per-model facts table (`providers/openai/model_facts.go`); IDs absent from the table keep numeric limits at zero and flags false without inventing values.
...
- Optional `StreamingTool`/`EventToolProgress` emit bounded incremental tool output; rich `ToolResult` (string `Metadata`, model `JSON`, truncation/compression/`SourceRef`/`Opaque`) is accepted from helpers and custom tools with `ValidateToolResult` before events/replay.
